...
function ExistinghospitalFormModal(props) {
  const [saveLoading, setSaveLoading] = useState(false);

  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ExistinghospitalService.create(data);
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      >
        <h2 style={{ margin: 0 }}>
          {i18n('entities.existinghospital.new.title')}
        </h2>
...
export default ExistinghospitalFormModal;
